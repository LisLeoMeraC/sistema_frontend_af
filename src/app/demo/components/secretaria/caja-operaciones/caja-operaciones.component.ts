...
@Component({
    ...
    styleUrls: ['./caja-operaciones.component.scss'],
})
export class CajaOperacionesComponent implements OnInit {
    menuItems!: MenuItem[];
    ...
    onSubmitOpenCaja(): void {
        if (this.openCajaForm.valid) {
            const formData = {
                ...this.openCajaForm.value,
                tipoCaja: 'OPERACIONES'
            };
            this.cajaService.abrirCaja(formData).subscribe({
                next: (response) => {
                    this.messageService.add({
                        severity: 'success',
                        summary: 'Éxito',
                        detail: 'Caja de Operaciones abierta correctamente',
                    });
    ...
    listarCaja(): void {
        this.cajaService.listarCajasDescendente('OPERACIONES').subscribe(
            (data: any[]) => {
